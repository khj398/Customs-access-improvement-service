...
def load_pbac_nos() -> list[str]:
    ...
    for filename in ("unipass_all_2b.json", "unipass_all_2c.json"):
        ...
        try:
            ...
            # 파일이 비어있어도(0 byte/invalid JSON) 오류로 보지 않고 건너뛴다.
            continue
...
def collect_single_pbac(page, pbac_no: str) -> bool:
    ...
    pbac_digits = digits_only(pbac_no)
    is_business = pbac_digits[-1] if len(pbac_digits) == 14 else "1"
...
def main():
    ...
    if not pbac_nos:
        print("수집할 공매번호가 없습니다. (unipass_all_2b.json / unipass_all_2c.json 확인)")
        return
